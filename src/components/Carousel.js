...
const Carousel = ({ images, vertical = false }) => {
  ...
  const updateCenterIndex = () => {
    ...
    if (carousel) {
      ...
      const centerCard = Math.floor(
        (carousel.scrollLeft + centerOffset) / cardWidth
  ...
  useEffect(() => {
    ...
    const leftEdge = carouselRef.current.querySelector(
      `.left-edge${vertical ? ".vertical" : ""}`
    );
    const rightEdge = carouselRef.current.querySelector(
      `.right-edge${vertical ? ".vertical" : ""}`
    );

    if (leftEdge && rightEdge) {
      leftEdge.addEventListener("mouseenter", () =>
        handleMouseEnter(vertical ? -1 : 1)
      );
    ...
    return () => {
      leftEdge &&
        leftEdge.removeEventListener("mouseenter", () =>
          handleMouseEnter(vertical ? -1 : 1)
        );
      leftEdge &&
        leftEdge.removeEventListener("mouseleave", () => clearInterval());
      rightEdge &&
        rightEdge.removeEventListener("mouseenter", () =>
          handleMouseEnter(vertical ? 1 : -1)
        );
      rightEdge &&
  ...
  return (
    <div className={`carousel-container ${vertical ? "vertical" : ""}`}>
      ...
      <div
        className={`carousel ${vertical ? "vertical" : ""}`}
        ref={carouselRef}
      >
      ...
      <div
        className={`arrow ${vertical ? "arrow-left" : "arrow-up"}`}
        onClick={() => handleScroll(vertical ? -1 : 1)}
      >
        &#9660;
      </div>
      <div
        className={`arrow ${vertical ? "arrow-right" : "arrow-down"}`}
        onClick={() => handleScroll(vertical ? 1 : -1)}
      >
        &#9650;
      </div>
